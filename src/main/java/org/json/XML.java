...
public class XML {
     ...
     *            The current nesting depth.
     * @param pathArray 
     *            Tracks the full path of the item to be searched for.
     * @param replacement
     *            The jsonobject to be inserted
     * 
     * @param canStartReplacement
     *            Tracks if we can start the replacement
     * 
     * @param pathMatching
     *            Tracks if parent path has been matched i.e. we're on a valid path
     * @return true if the close tag is processed.
     * 
     * @throws JSONException Thrown if any parsing error occurs.
     */
    private static boolean parse(XMLTokener x, JSONObject context, String name, XMLParserConfiguration config, int currentNestingDepth, String[] pathArray, JSONObject replacement, boolean canStartReplacement, boolean pathMatching)
            throws JSONException {
        char c;
        int i;
        JSONObject jsonObject = null;
        ...
        if (token == BANG) {
            ...
            if (c == '-') {
                ...
                if ("CDATA".equals(token)) {
                    if (x.next() == '[') {
                        string = x.nextCDATA();
                        if (string.length() > 0 && !canStartReplacement) {
                            context.accumulate(config.getcDataTagName(), string);
                        }
                        return false;
                    }
        ...
        } else if (token instanceof Character && !canStartReplacement) {
            throw x.syntaxError("Misshaped tag");

            // Open tag <

        } else {
            // Process a well-formed tag.
            tagName = (String) token;
            ...
            boolean nilAttributeFound = false;
            xmlXsiTypeConverter = null;
            // Handles processing of the rest of the tag (>, attributes)
            for (;;) {
                if (token == null) {
                    token = x.nextToken();
                }
                ...
                if (token instanceof String) {
                    ...
                    for (;;) {
                        ...
                        if (token == null) {
                            ...
                            if (parse(x, jsonObject, tagName, config, currentNestingDepth + 1, pathArray, replacement, canReplaceCurrent, currentlyMatches)) {
                                if (canReplaceCurrent) {
                                ...
                                // "{"contact":{"nick":"Crista","address":{"zipcode":92614,"street":"Ave of the Arts","content":">"},"name":"Crista Lopes"}}"
                                return false;
                            }
                        }
                    }
     ...
     *            Tracks the full path of the item to be searched for.
     * 
     * @param canStartBuilding
     *            Tracks if we can start the accumulation of the object of interest
     * 
     * @param pathMatching
     *            Tracks if parent path has been matched i.e. we're on a valid path
     * 
     * @param matched
     *            Tracks if we've already added the object of interest (and can therefore skip).
     * @return true if the close tag is processed.
     * 
     * @throws JSONException Thrown if any parsing error occurs.
     */
    ...
    private static boolean parse(XMLTokener x, JSONObject context, String name, XMLParserConfiguration config, 
                                  int currentNestingDepth, String[] pathArray, boolean canStartBuilding, 
                                  boolean pathMatching, boolean[] matched)
            throws JSONException {
        char c;
        int i;
        JSONObject jsonObject = null;
        ...
        if (token == BANG) {
            ...
            // Set the tagname to be what the current token is.
            tagName = (String) token;
            // Tracks if current tag is in the path. If it looks good, and all parent nodes have been matched, this is true.
            boolean currentlyMatches = pathMatching;
            // Tells parser to start constructing output. Should only be true when we're on the last level of pathArr.

            boolean canBuildCurrent = canStartBuilding;

            // currentlyMatches should be true if we're going in with a valid path, and our current tag is also valid.
            if (currentNestingDepth < pathArray.length) {
                if (currentNestingDepth == 0) {
                    currentlyMatches = tagName.equals(pathArray[currentNestingDepth]);
                } else {
                ...
                // If we're at the end of the path array, we can start building
                if (currentNestingDepth == pathArray.length - 1 && currentlyMatches && !matched[0]) {
            ...
            for (;;) {
                ...
                if (token instanceof String) {
                    ...
                    } else if (canBuildCurrent && !matched[0]) {
                        // Handles the case where there's just a regular ass string
                        jsonObject.accumulate(string, "");
                    }


                } else if (token == SLASH) {
                    // Handles tokens that look like: <element /> (self closing)
                    if (x.nextToken() != GT) {
                        throw x.syntaxError("Misshaped tag");
                    }
                    if (canBuildCurrent && !matched[0]) {
                        if (config.getForceList().contains(tagName)) {
                            // Force the value to be an array
                            if (nilAttributeFound) {
                                context.append(tagName, JSONObject.NULL);
                        ...
                        if (currentNestingDepth == pathArray.length - 1) {
                            // If we're at the correct depth to start collecting, but all we found was this empty tag.
                            // We should terminate, and return nothing.
                            matched[0] = true;
                        }
                    }
                    ...
                    for (;;) {
                        ...
                        if (token == null) {
                            ...
                            // If we have regular content contained in tags
                            string = (String) token;
                            if (string.length() > 0 && canBuildCurrent && !matched[0]) {  
                                if(xmlXsiTypeConverter != null) {
                                    jsonObject.accumulate(config.getcDataTagName(),
                                            stringToValue(string, xmlXsiTypeConverter));
                                } else {
                            ...
                            if (parse(x, jsonObject, tagName, config, currentNestingDepth + 1, pathArray, canBuildCurrent, currentlyMatches, matched)) {
                                if (canBuildCurrent && !matched[0]) {
                                    // If we're good to start storing, and we haven't completely found what we're looking for yet.
                                    if (!config.getForceList().contains(tagName)) {
                                        if (jsonObject.length() == 0) {
                                            context.accumulate(tagName, "");
                                        } else if (jsonObject.length() == 1
                                    ...
                                    if (currentNestingDepth == pathArray.length - 1) {
                                        // We found a hit, change the flag to ignore.
                                        matched[0] = true;
                                    }
    ...
    private static boolean parse(XMLTokener x, JSONObject context, String name, XMLParserConfiguration config, int currentNestingDepth, KeyTransformerInterface keyTransformer)
        ...
        if (token == BANG) {
            ...
            if (!(applyTransformation(token.toString(), keyTransformer)).equals(name)) {
                throw x.syntaxError("Mismatched " + name + " and " + token);
            }
            if (x.nextToken() != GT) {
                throw x.syntaxError("Misshaped close tag");
     ...
     *
     * @param reader The XML source reader.
     * @pointer A path that specifies the location of a JSONObject.
     * @return A JSONObject containing the structured data pointed to by the given path.
     * @throws JSONException Thrown if there is an errors while parsing the string
     */
     ...
     * @param reader The XML source reader.
     * @pointer A path that specifies the location of a JSONObject.
     * @replacementObj The JSONObject to be substituted in at the specified location.
     * @return A JSONObject containing the structured data with the replacement.
     * @throws JSONException Thrown if there is an errors while parsing the string
     */
    public static JSONObject toJSONObject(Reader reader, JSONPointer pointer, JSONObject replacementObj) throws JSONException{
        return toJSONObject(reader, XMLParserConfiguration.ORIGINAL, pointer, replacementObj);
    }
    ...
    /**
     * Helper function that validates the output of a user-provided keyTransformer and
     * applies the transformation if so. If not, it will leave the string as-is.
     * 
     * @param reader The XML source reader.
     * @return A JSONObject containing the structured data pointed to by the given path.
     * @throws JSONException Thrown if there is an errors while parsing the string
    ...
    public static JSONObject toJSONObject(Reader reader, XMLParserConfiguration config, JSONPointer pointer) throws JSONException {
        ...
        while (x.more() && !match) {
            x.skipPast("<");
            if(x.more()) {
                boolean[] matching = new boolean[]{match};
                parse(x, jo, null, config, 0, pathArray, false, false, matching);
            }
        }
        return jo;
    } 
    ...
    public static JSONObject toJSONObject(Reader reader, XMLParserConfiguration config, JSONPointer pointer, JSONObject replacementObj) throws JSONException {
        ...
        while (x.more()) {
            x.skipPast("<");
            if(x.more()) {
                parse(x, jo, null, config, 0, pathArray, replacementObj, false, false);
            }
        }
        return jo;
    } 
